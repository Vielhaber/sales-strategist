...
function init() {
  ...
  // Landing Page Generator DOM Elements
  const landingpageForm = document.getElementById("landingpage-form");
  const lpProductNameInput = document.getElementById("lp-product-name");
  const lpStyleSelect = document.getElementById("lp-style");
  const lpCtaInput = document.getElementById("lp-cta");
  const lpPromptInput = document.getElementById("lp-prompt");
  const btnLpUseContext = document.getElementById("btn-lp-use-context");
  const btnLpGenerate = document.getElementById("btn-lp-generate");
  const lpLoading = document.getElementById("lp-loading");
  const lpLoadingText = document.getElementById("lp-loading-text");
  const lpResultContainer = document.getElementById("lp-result-container");
  const btnLpViewPreview = document.getElementById("btn-lp-view-preview");
  const btnLpViewCode = document.getElementById("btn-lp-view-code");
  const lpPreviewView = document.getElementById("lp-preview-view");
  const lpCodeView = document.getElementById("lp-code-view");
  const lpPreviewFrame = document.getElementById("lp-preview-frame");
  const lpCodeOutput = document.getElementById("lp-code-output");
  const btnLpCopy = document.getElementById("btn-lp-copy");
  const btnLpDownload = document.getElementById("btn-lp-download");
  ...
  let lpGeneratedHtml = ""; // Last landing page HTML generated by the Landingpage tab
  ...
  // --- KI LANDINGPAGE-GENERATOR ---

  if (btnLpUseContext) {
    btnLpUseContext.addEventListener("click", () => {
      if (productInput.value.trim()) lpProductNameInput.value = productInput.value.trim();
      if (!lpPromptInput.value.trim() && (industryInput.value.trim() || productInput.value.trim())) {
        lpPromptInput.value = `Landingpage für "${productInput.value.trim() || "unser Produkt"}", Zielgruppe: ${industryInput.value.trim() || "B2B-Kunden"}.`;
      }
    });
  }

  if (landingpageForm) {
    landingpageForm.addEventListener("submit", async (e) => {
      e.preventDefault();

      const fields = {
        productName: lpProductNameInput.value.trim(),
        style: lpStyleSelect.value,
        cta: lpCtaInput.value.trim(),
        prompt: lpPromptInput.value.trim(),
      };

      lpResultContainer.style.display = "none";
      lpLoading.style.display = "flex";
      btnLpGenerate.disabled = true;

      try {
        let html;
        if (!apiKeyConfigured) {
          lpLoadingText.innerText = "Baue Offline-Vorschau...";
          html = generateLocalMockLandingPage(fields);
        } else {
          lpLoadingText.innerText = "KI entwirft deine Landingpage...";
          const selectedModel = modelSelect ? modelSelect.value : "gemini-2.5-flash";
          const prompt = buildLandingPagePrompt(fields);
          const rawResponse = await apiGenerate(prompt, selectedModel);
          html = extractHtmlFromResponse(rawResponse);
        }

        showLandingPageResult(html);
      } catch (err) {
        console.error("Landing page generation failed:", err);
        const html = generateLocalMockLandingPage(fields);
        showLandingPageResult(html);
        alert(`Hinweis: Die Live-Generierung ist fehlgeschlagen, daher wurde eine Offline-Vorschau gebaut:\n\n${err.message || err}`);
      } finally {
        lpLoading.style.display = "none";
        btnLpGenerate.disabled = false;
      }
    });
  }

  function showLandingPageResult(html) {
    lpGeneratedHtml = html;
    lpPreviewFrame.srcdoc = html;
    lpCodeOutput.value = html;
    lpResultContainer.style.display = "block";
    switchLandingPageView("preview");
  }

  function switchLandingPageView(view) {
    const showPreview = view === "preview";
    if (btnLpViewPreview) btnLpViewPreview.classList.toggle("active", showPreview);
    if (btnLpViewCode) btnLpViewCode.classList.toggle("active", !showPreview);
    if (lpPreviewView) lpPreviewView.style.display = showPreview ? "block" : "none";
    if (lpCodeView) lpCodeView.style.display = showPreview ? "none" : "block";
  }

  if (btnLpViewPreview) btnLpViewPreview.addEventListener("click", () => switchLandingPageView("preview"));
  if (btnLpViewCode) btnLpViewCode.addEventListener("click", () => switchLandingPageView("code"));

  if (btnLpCopy) {
    btnLpCopy.addEventListener("click", () => {
      if (!lpGeneratedHtml) return;
      navigator.clipboard.writeText(lpGeneratedHtml).then(() => {
        const original = btnLpCopy.innerHTML;
        btnLpCopy.innerHTML = `<i data-lucide="check" style="width: 14px; height: 14px; color: var(--color-success)"></i> Kopiert!`;
        lucide.createIcons();
        setTimeout(() => {
          btnLpCopy.innerHTML = original;
          lucide.createIcons();
        }, 2000);
  ...
  if (btnLpDownload) {
    btnLpDownload.addEventListener("click", () => {
      if (!lpGeneratedHtml) return;
      const blob = new Blob([lpGeneratedHtml], { type: "text/html;charset=utf-8;" });
      const link = document.createElement("a");
      link.href = URL.createObjectURL(blob);
